...
sum(case when x.order_date between date_format(now() - interval 1 month, "%2023-%m-01") and date_format(now() - interval 1 month, "%2023-%m-31") then Live_Prducts else 0 end) as lm_live_Products,
sum(case when x.order_date between date_format(now() - interval 1 month, "%2023-%m-01") and date_format(now() - interval 1 month, "%2023-%m-31") then Live_Stores else 0 end) as lm_new_stores
